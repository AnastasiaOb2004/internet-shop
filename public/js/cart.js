const createSmallCards = (data) =>{
    ...
    <div class="sm-product">
                    ...
                    <p class="sm-price" data-price="${data.sellingPrice}">${data.sellingPrice * data.item} SEK</p>
...
const setProducts = (name) => {
    ...
    if (!data || data.length === 0) {
        element.innerHTML = `
            <img src="images/empty-cart.png" class="empty-img" alt="">
        `;
    } else {
...
const updateBill = () =>{
    let billPrice = document.querySelector('.bill');
        billPrice.innerHTML = `${totalBill} SEK`;
}
    ...
    counts.forEach((item,i) => {
        ...
        counterMinus[i].addEventListener('click', () =>{
            if(item.innerHTML >1){
                item.innerHTML--;
                totalBill -= cost;
                price[i].innerHTML = `${item.innerHTML * cost} SEK`;
